...
impl PackageBackend for MambaBackend {
    ...
    async fn get_changelog(&self, name: &str) -> Result<Option<String>> {
        let output = Command::new("mamba")
            ...
            .output()
            .await
            .context("Failed to search mamba packages for changelog")?;
        ...
        if unique_releases.len() > 25 {
            changelog.push_str(&format!(
                "*...and {} more versions on mamba channels*\n",
...
mod tests {
    ...
    #[cfg(target_os = "linux")]
    fn create_fake_mamba_script() -> (
        std::path::PathBuf,
        std::path::PathBuf,
        Option<std::ffi::OsString>,
    ) {
        let fixture_dir = std::env::temp_dir().join(format!(
            "linget-mamba-dry-run-test-{}-{}",
            std::process::id(),
            SystemTime::now()
        ...
        let mut script = File::create(&script_path).expect("create fake mamba script");
        writeln!(script, "#!/usr/bin/env sh").expect("write fake mamba script");
        writeln!(script, "echo \"$*\" >> \"{}\"", log_path.to_string_lossy())
            .expect("write fake mamba script");
        writeln!(script, "if [ \"$1\" = \"list\" ]; then").expect("write fake mamba script");
        script
            .write_all(b"  echo '[{\"name\":\"pkg-orphan\",\"version\":\"1.0\"}]'\n")
            .expect("write fake mamba script");
        writeln!(script, "elif [ \"$1\" = \"update\" ]; then").expect("write fake mamba script");
        script
            .write_all(
                b"  echo '{\"actions\":{\"LINK\":[{\"name\":\"pkg-orphan\",\"version\":\"2.0\"}]}}'\n",
            )
            .expect("write fake mamba script");
        writeln!(script, "elif [ \"$1\" = \"search\" ]; then").expect("write fake mamba script");
        script
            .write_all(
            ...
            .expect("write fake mamba script");
        writeln!(script, "else").expect("write fake mamba script");
        script
            .write_all(b"  echo '{}'\n")
            .expect("write fake mamba script");
        writeln!(script, "fi").expect("write fake mamba script");
    ...
    async fn test_check_updates_uses_dry_run_and_json() {
        ...
        let parsed_updates: serde_json::Value =
            serde_json::from_slice(&update_output).expect("script update output should parse");
        ...
        let installed = backend
            .list_installed()
            .await
            .expect("installed mamba packages");
        ...
        let updates = result.expect("mamba updates");
    ...
    async fn test_get_changelog_builds_version_timeline_from_search() {
        ...
        let changelog = backend
            .get_changelog("demo")
            .await
            .expect("mamba changelog call")
            .expect("expected synthetic mamba changelog");
