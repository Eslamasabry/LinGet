...
fn provider_row(source: PackageSource) -> ProviderStatus {
    let probe = match source {
        ...
        PackageSource::Winget => ProviderProbe {
            list_cmds: &["winget"],
            version_cmd: Some(("winget", &["--version"])),
        },
        PackageSource::Chocolatey => ProviderProbe {
            list_cmds: &["choco"],
            version_cmd: Some(("choco", &["--version"])),
        },
        PackageSource::Scoop => ProviderProbe {
            list_cmds: &["scoop"],
            version_cmd: Some(("scoop", &["--version"])),
        },
    };

    let privileged_cmds = source.privileged_probe_commands();

    let mut found_paths = which_all(probe.list_cmds);
    let privileged_paths = which_all(privileged_cmds);
    ...
    ProviderStatus {
        ...
        privileged_cmds: privileged_cmds.iter().map(|s| s.to_string()).collect(),
        found_paths,
        version,
        reason,
    }
...
mod tests {
    ...
    fn test_detect_single_provider() {
        let providers = detect_providers();
        let primary_source = if cfg!(target_os = "windows") {
        ...
        let provider_status = providers
            ...
            .expect("primary provider should be in list");
        assert_eq!(provider_status.source, primary_source);
        assert_eq!(provider_status.display_name, primary_source.to_string());
        assert!(!provider_status.list_cmds.is_empty());
    }
