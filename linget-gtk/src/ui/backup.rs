    ...
    glib::spawn_future_local(async move {
        ...
        for pkg in &packages {
            ...
            match manager.install(&stub_pkg).await {
                Ok(_) => installed += 1,
                Err(e) => {
                    tracing::warn!("Failed to install {}: {}", pkg.name, e);
                    failed_packages.push((pkg.name.clone(), e.to_string()));
                }
            }
        }
        ...
        if let Some(overlay) = &toast_overlay {
            ...
            let message = if failed_count > 0 {
                let failed_names: Vec<&str> = failed_packages
                    .iter()
                    .take(3)
                    .map(|(n, _)| n.as_str())
                    .collect();
                let names_str = failed_names.join(", ");
                let suffix = if failed_count > 3 {
                    format!(" and {} more", failed_count - 3)
                } else {
                    String::new()
                };
                format!(
                    "Import: {} installed, {} failed ({}{})",
                    installed, failed_count, names_str, suffix
                )
            } else {
