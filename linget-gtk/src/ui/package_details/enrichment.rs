...
pub fn build_section(
    ...
    if !enrichment.screenshots.is_empty() {
        ...
        for url in enrichment.screenshots.iter().take(5) {
            // Stack to show spinner while loading, then image when ready
            let image_stack = gtk::Stack::builder()
            ...
            let spinner = gtk::Spinner::builder()
                ...
                .height_request(32)
                .build();
            let loading_label = gtk::Label::builder().label("Loading screenshot...").build();
            ...
            image_stack.add_named(&loading_box, Some("loading"));
            image_stack.add_named(&image, Some("image"));
            image_stack.set_visible_child_name("loading");

            let url = url.clone();
            let image_clone = image.clone();
            let stack_clone = image_stack.clone();
