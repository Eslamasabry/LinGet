...
pub struct AppModel {
    ...
    pub preparing_action_preview: bool,
...
impl SimpleComponent for AppModel {
    ...
    fn init(
        ...
        let model = AppModel {
            ...
            preparing_action_preview: false,
    ...
    fn update(&mut self, msg: Self::Input, sender: ComponentSender<Self>) {
        match msg {
                        ...
                        result = fut => {
                            match result {
                                ...
                                Err(e) => {
                                    ...
                                    sender.input(AppMsg::UpdateCheckFailed(e.to_string()));
                                }
                            }
                        }
                    }
            ...
            AppMsg::UpdateCheckFailed(error) => {
                self.checking_updates = false;
                sender.input(AppMsg::ShowToast(
            ...
            AppMsg::PackageAction(pkg) => {
                ...
                if action_type == ActionType::Remove {
                    self.preparing_action_preview = true;
                    let pm = self.package_manager.clone();
            ...
            AppMsg::PrepareRemovePreview {
                ...
                self.preparing_action_preview = false;
                let preview =
            ...
            AppMsg::InitializeHistoryTracker => {
                ...
                relm4::spawn(async move {
                    match HistoryTracker::load().await {
                        ...
                        Err(e) => {
                            tracing::error!(error = %e, "Failed to load history tracker, history feature disabled");
                            // Leave history tracker as None - feature will be disabled
                            sender.input(AppMsg::ShowToast(
                                "History tracking unavailable".to_string(),
                                ToastType::Warning,
                            ));
                        }
                    }
